...
def run_regression_test(X, y, ntrials=2, min_training_score = .85, min_samples_leaf=3, max_features=0.3, grace=.08, oob=False, n_estimators=18):
    ...
    for i in range(ntrials):
        ...
        rf = RandomForestRegressor621(n_estimators=n_estimators, min_samples_leaf=min_samples_leaf, max_features=max_features)
        rf.fit(X_train, y_train)
        train_scores.append(rf.score(X_train, y_train))
        test_scores.append(rf.score(X_test, y_test))

        sklearn_rf = RandomForestRegressor(n_estimators=n_estimators, min_samples_leaf=min_samples_leaf, max_features=max_features)
...
def run_classification_test(X, y, ntrials=1, min_samples_leaf=3, max_features=0.3, min_training_score=1.0, grace=.07, oob=False, n_estimators=15):
    ...
    for i in range(ntrials):
        ...
        rf = RandomForestClassifier621(n_estimators=n_estimators, min_samples_leaf=min_samples_leaf, max_features=max_features)
        rf.fit(X_train, y_train)
        train_scores.append(rf.score(X_train, y_train))
        test_scores.append(rf.score(X_test, y_test))

        sklearn_rf = RandomForestClassifier(n_estimators=n_estimators, min_samples_leaf=min_samples_leaf, max_features=max_features)
        sklearn_rf.fit(X_train, y_train)
        sklearn_train_scores.append(sklearn_rf.score(X_train, y_train))
